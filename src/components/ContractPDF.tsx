...
import { type SystemType, type Contract, formatCurrency } from '@/lib/mock-data';
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div ref={printRef} className="text-xs leading-relaxed text-foreground">
          <div className="contract-header" style={{ textAlign: 'center', borderBottom: '2px solid #f97316', paddingBottom: '16px', marginBottom: '20px' }}>
          ...
          <div className="clause" style={{ marginBottom: '14px' }}>
            ...
            <div className="highlight" style={{ background: '#fff7ed', padding: '8px 12px', borderRadius: '6px', borderLeft: '3px solid #f97316', margin: '8px 0' }}>
          ...
          <div className="signatures" style={{ marginTop: '40px', display: 'flex', justifyContent: 'space-between', gap: '40px' }}>
            <div className="sig-box" style={{ flex: 1, textAlign: 'center', borderTop: '1px solid #333', paddingTop: '8px' }}>
              {showSignatures && contract.signatures.length > 0 ? (
                <>
                  <p className="sig-name" style={{ fontWeight: 700, fontSize: '11px' }}>{contract.signatures[0].name}</p>
                  <p style={{ fontSize: '10px' }}>CPF/CNPJ: {contract.signatures[0].document}</p>
                  <p className="sig-filled" style={{ color: '#16a34a', fontSize: '9px' }}>
                    ✓ Assinado em {new Date(contract.signatures[0].signedAt).toLocaleDateString('pt-BR')}
                  </p>
                </>
              ) : (
                <>
                  <p className="sig-name" style={{ fontWeight: 700, fontSize: '11px' }}>{contract.clientName}</p>
            ...
            <div className="sig-box" style={{ flex: 1, textAlign: 'center', borderTop: '1px solid #333', paddingTop: '8px' }}>
              {showSignatures && contract.signatures.length > 1 ? (
                <>
                  <p className="sig-name" style={{ fontWeight: 700, fontSize: '11px' }}>{contract.signatures[1].name}</p>
                  <p style={{ fontSize: '10px' }}>CPF/CNPJ: {contract.signatures[1].document}</p>
                  <p className="sig-filled" style={{ color: '#16a34a', fontSize: '9px' }}>
                    ✓ Assinado em {new Date(contract.signatures[1].signedAt).toLocaleDateString('pt-BR')}
                  </p>
                </>
              ) : (
                <>
                  <p className="sig-name" style={{ fontWeight: 700, fontSize: '11px' }}>Inforsol Energia Solar Ltda</p>
          ...
          <div className="footer" style={{ marginTop: '30px', textAlign: 'center', borderTop: '1px solid #ddd', paddingTop: '10px', fontSize: '9px', color: '#888' }}>
